...
function reportScanProgress(label) {
    figma.ui.postMessage({ type: 'scan-progress', label });
}
async function yieldDuringScan() {
    await new Promise((resolve) => setTimeout(resolve, 0));
    if (scanCancelled)
        throw new Error('__SCAN_CANCELLED__');
}
function getNodesToScan(scope) {
...
function buildNodeIndex(nodes) {
    ...
    for (const node of nodes) {
        ...
        if ('fills' in node || 'strokes' in node)
            index.paintable.push(node);
        if ('layoutMode' in node && node.layoutMode !== 'NONE')
            index.autoLayout.push(node);
        if ('cornerRadius' in node && typeof node.cornerRadius === 'number')
            index.rounded.push(node);
        if ('width' in node && 'height' in node)
            index.sized.push(node);
        if (node.type === 'COMPONENT')
            index.components.push(node);
        if (node.type === 'INSTANCE')
            index.instances.push(node);
    }
    return index;
}
...
function colorsMatch(a, b, threshold = 0.001) {
    ...
    for (let i = 0; i < repeated.length; i += 1) {
        ...
        for (let j = i + 1; j < repeated.length; j += 1) {
            ...
            issues.push({
                ...
                description: `${inconsistent.records.length} affected layer${inconsistent.records.length === 1 ? '' : 's'} use ${sourceHex}, ` +
...
function scanSpacing(nodes) {
    ...
    for (const kind of ['gap', 'padding']) {
        ...
        for (const [value, count] of sorted) {
            if (value === dominantValue || Math.abs(value - dominantValue) > 4)
                continue;
...
function scanTypography(nodes) {
    ...
    for (const [key, group] of groups) {
        ...
        for (const record of group)
            counts.set(record.size, (counts.get(record.size) ?? 0) + 1);
        ...
        for (const [size, count] of sizes) {
            if (size === dominantSize || Math.abs(size - dominantSize) > 4)
                continue;
            ...
            issues.push({
                ...
                description: `${affected.length} text layer${affected.length === 1 ? '' : 's'} use ${size}px ` +
...
function scanRadii(nodes) {
    ...
    const issues = [];
    for (const [value, count] of sorted) {
        if (value === dominantValue || Math.abs(value - dominantValue) > 2)
            continue;
...
function scanButtons(nodes) {
    const records = [];
    for (const node of nodes) {
        if (!('height' in node) || !('width' in node))
            continue;
        if (!/(button|btn|cta)/i.test(node.name))
            continue;
        records.push({
    ...
    const frequency = new Map();
    for (const record of records)
        frequency.set(record.height, (frequency.get(record.height) ?? 0) + 1);
    const sorted = [...frequency.entries()].sort((a, b) => b[1] - a[1] || a[0] - b[0]);
    const dominantHeight = sorted[0][0];
    const dominantFrequency = sorted[0][1];
    const issues = [];
    for (const [height, count] of sorted) {
        if (height === dominantHeight || Math.abs(height - dominantHeight) > 8)
            continue;
...
async function scanComponents(components, instances) {
    const groups = new Map();
    let componentNodeIndex = 0;
    for (const node of components) {
        componentNodeIndex += 1;
        if (componentNodeIndex % 250 === 0)
            await yieldDuringScan();
    ...
    const referencedComponentIds = new Set(instanceResults.map((result) => result.mainComponent?.id).filter((id) => Boolean(id)));
...
async function scanNaming(nodes) {
    const issues = [];
    ...
    for (const node of nodes) {
        namingNodeIndex += 1;
        if (namingNodeIndex % 250 === 0)
            await yieldDuringScan();
        if (node.type === 'TEXT' || node.type === 'GROUP' || node.type === 'VECTOR')
            continue;
        const parentId = node.parent?.id ?? 'root';
        const names = namesByParent.get(parentId) ?? new Map();
    ...
    for (const names of namesByParent.values()) {
        for (const [name, matchingNodes] of names) {
            if (!name || matchingNodes.length < 2 || genericNamePattern.test(name))
                continue;
            issues.push({
...
async function scanAccessibility(nodes) {
    const issues = [];
    let accessibilityNodeIndex = 0;
    for (const node of nodes) {
        accessibilityNodeIndex += 1;
        if (accessibilityNodeIndex % 250 === 0)
            await yieldDuringScan();
        ...
        while (ancestor && ancestor.type !== 'PAGE' && ancestor.type !== 'DOCUMENT') {
            background = getSolidFill(ancestor);
            if (background)
                break;
...
function dimensionLayerType(node) {
    switch (node.type) {
        case 'COMPONENT': return 'component';
        case 'INSTANCE': return 'component instance';
...
async function scanDimensions(nodes) {
    const byType = new Map();
    let dimensionNodeIndex = 0;
    for (const node of nodes) {
        dimensionNodeIndex += 1;
        if (dimensionNodeIndex % 250 === 0)
            await yieldDuringScan();
        if (!('width' in node) || !('height' in node))
            continue;
    ...
    for (const [layerType, typeNodes] of byType) {
        ...
        for (const [dimension, matchingNodes] of sorted) {
            if (dimension === dominant[0] || matchingNodes.length < 2)
                continue;
...
function isSafeDimensionNode(node) {
    if (!('resize' in node) || !('width' in node) || !('height' in node))
        return false;
    if (node.type === 'COMPONENT' || node.type === 'INSTANCE')
        return false;
    if (isLayoutManagedDimension(node))
        return false;
    if (node.type === 'TEXT')
        return node.textAutoResize === 'NONE';
    return ['FRAME', 'SECTION', 'RECTANGLE', 'ELLIPSE', 'VECTOR', 'LINE'].includes(node.type);
}
function isLayoutManagedDimension(node) {
    if ('layoutMode' in node && node.layoutMode !== 'NONE') {
...
async function scanStyles(nodes) {
    ...
    const consumersByStyleId = new Map();
    ...
    for (const duplicateStyles of duplicateGroups.values()) {
        ...
        if (duplicateGroupIndex % 10 === 0)
            await yieldDuringScan();
        if (duplicateStyles.length < 2)
            continue;
        const canonical = [...duplicateStyles].sort((a, b) => (consumersByStyleId.get(b.id)?.length ?? 0) - (consumersByStyleId.get(a.id)?.length ?? 0))[0];
    ...
    for (const style of allStyles) {
        if ((consumersByStyleId.get(style.id) ?? []).length > 0)
            continue;
    ...
    for (const style of paintStyles) {
        paintStyleIndex += 1;
        if (paintStyleIndex % 10 === 0)
            await yieldDuringScan();
        const styleColor = getStyleSolidColor(style);
        if (!styleColor)
            continue;
        const affected = nodes.filter((node) => {
            if (!('fills' in node) || !('fillStyleId' in node) || !('setFillStyleIdAsync' in node))
                return false;
            if (node.fillStyleId !== '')
                return false;
            return Array.isArray(node.fills) && node.fills.some((paint) => paint.type === 'SOLID' && paint.visible !== false && colorsMatch(paint.color, styleColor, 0.001));
        });
        if (affected.length === 0)
            continue;
    ...
    for (const style of textStyles) {
        textStyleIndex += 1;
        if (textStyleIndex % 10 === 0)
            await yieldDuringScan();
        const affected = nodes.filter((node) => {
            if (node.type !== 'TEXT' || !('textStyleId' in node) || !('setTextStyleIdAsync' in node))
                return false;
            if (node.textStyleId !== '' || node.fontName === figma.mixed || typeof node.fontSize !== 'number')
                return false;
            return fontNamesMatch(node.fontName, style.fontName) && Math.abs(node.fontSize - style.fontSize) < 0.001;
        });
        if (affected.length === 0)
            continue;
        const nodeIds = affected.map((node) => node.id);
...
async function scanVariables(nodes) {
    ...
    const collectAliasIds = (value) => {
        if (!value)
            return;
        if (Array.isArray(value)) {
            value.forEach(collectAliasIds);
            return;
        }
        ...
        if (record.type === 'VARIABLE_ALIAS' && typeof record.id === 'string')
            usedVariableIds.add(record.id);
        Object.values(record).forEach(collectAliasIds);
    };
    const addMatch = (variableId, variableName, field, binding) => {
    ...
    for (const node of nodes) {
        ...
        if (inferredFills) {
            for (let paintIndex = 0; paintIndex < inferredFills.length; paintIndex += 1) {
                for (const alias of inferredFills[paintIndex] ?? []) {
                    ...
                    if (variable) {
                        addMatch(variable.id, variable.name, 'fill', { nodeId: node.id, paintIndex });
                        break;
                    }
                }
            }
        ...
        if (inferredStrokes) {
            for (let paintIndex = 0; paintIndex < inferredStrokes.length; paintIndex += 1) {
                for (const alias of inferredStrokes[paintIndex] ?? []) {
                    const variable = colorById.get(alias.id);
                    if (variable) {
                        addMatch(variable.id, variable.name, 'stroke', { nodeId: node.id, paintIndex });
                        break;
                    }
                }
            }
        ...
        for (const { field } of numericFields) {
            if ((field === 'width' || field === 'height') && !isSafeDimensionNode(node))
                continue;
            const alias = inferred[field];
            const variable = alias ? floatById.get(alias.id) : undefined;
            if (variable)
                addMatch(variable.id, variable.name, field, { nodeId: node.id, field });
        }
    }
    ...
    for (const variable of [...localColorVariables, ...localFloatVariables]) {
        ...
        if (unusedVariableIndex % 25 === 0)
            await yieldDuringScan();
        if (usedVariableIds.has(variable.id) || matches.has(`${variable.id}\u0000fill`) || matches.has(`${variable.id}\u0000stroke`))
            continue;
        issues.push({
...
async function getVariableCoverage(nodes) {
    ...
    for (const node of nodes) {
        coverageNodeIndex += 1;
        if (coverageNodeIndex % 250 === 0)
            await yieldDuringScan();
        if ('fills' in node && Array.isArray(node.fills)) {
            for (let index = 0; index < node.fills.length; index += 1) {
                if (node.fills[index].type !== 'SOLID')
                    continue;
                tokenizableValues += 1;
        ...
        if ('strokes' in node && Array.isArray(node.strokes)) {
            for (let index = 0; index < node.strokes.length; index += 1) {
                if (node.strokes[index].type !== 'SOLID')
                    continue;
                tokenizableValues += 1;
                if (node.boundVariables?.strokes?.[index])
                    boundValues += 1;
            }
        }
...
async function getFullDocumentNodes() {
    await figma.loadAllPagesAsync();
    const nodes = [];
    for (const page of figma.root.children)
        nodes.push(...await collectNodes(page.children));
    return nodes;
}
async function scanLayout(nodes) {
    const issues = [];
    let layoutNodeIndex = 0;
    for (const parent of nodes) {
        layoutNodeIndex += 1;
        if (layoutNodeIndex % 250 === 0)
            await yieldDuringScan();
        if (!('layoutMode' in parent) || parent.layoutMode === 'NONE' || !('children' in parent))
            continue;
        ...
        if (horizontal.size <= 1 && vertical.size <= 1)
            continue;
        const horizontalModes = [...horizontalCounts.entries()].sort((a, b) => b[1] - a[1]).map(([mode]) => mode);
        const verticalModes = [...verticalCounts.entries()].sort((a, b) => b[1] - a[1]).map(([mode]) => mode);
...
function calculateChaosScore(issues, nodeCount) {
...
async function getSceneNode(nodeId) {
...
async function applyColorFix(fix) {
    let changed = 0;
...
async function applyAccessibilityFix(fix) {
    ...
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node || node.type !== 'TEXT' || !Array.isArray(node.fills) || !('setFillsAsync' in node))
            continue;
...
async function applyTypographyFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
...
async function applySpacingFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node)
...
async function applyRadiusFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node || !('cornerRadius' in node) || typeof node.cornerRadius !== 'number')
            continue;
...
async function applyButtonFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node || !('width' in node) || !('height' in node) || !('resize' in node))
            continue;
...
async function applyComponentFix(fix) {
    const canonical = await getSceneNode(fix.canonicalId);
    if (!canonical || canonical.type !== 'COMPONENT')
        return 0;
    let changed = 0;
    ...
    for (const instanceId of instanceIds) {
        ...
        if (!mainComponent || !fix.duplicateIds.includes(mainComponent.id))
            continue;
        if (mainComponent.id === fix.canonicalId)
            continue;
    ...
    if (fix.mode === 'merge' || fix.mode === 'instance') {
        for (const duplicateId of fix.duplicateIds) {
            if (duplicateId === fix.canonicalId)
                continue;
            const duplicate = await figma.getNodeByIdAsync(duplicateId);
            if (!duplicate || duplicate.type !== 'COMPONENT')
                continue;
            if (fix.mode === 'instance') {
                ...
                if ('rotation' in duplicate)
                    replacement.rotation = duplicate.rotation;
                if ('opacity' in duplicate)
                    replacement.opacity = duplicate.opacity;
                if ('visible' in duplicate)
                    replacement.visible = duplicate.visible;
                if ('width' in duplicate && 'height' in duplicate && 'resize' in replacement) {
                    const resizable = replacement;
...
async function renameDuplicateComponents(nodeIds, canonicalId, baseName) {
    let changed = 0;
...
async function applyNamingFix(fix) {
    let changed = 0;
...
async function applyDimensionsFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node || !isSafeDimensionNode(node))
            continue;
        const resizable = node;
...
async function applyLayoutFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node || !('layoutSizingHorizontal' in node) || !('layoutSizingVertical' in node))
            continue;
        let didChange = false;
...
async function applyStyleFix(fix) {
    let changed = 0;
    for (const nodeId of fix.nodeIds) {
        const node = await getSceneNode(nodeId);
        if (!node)
...
async function applyVariableFix(fix) {
    ...
    if (fix.variableField !== 'fill' && fix.variableField !== 'stroke') {
        ...
        for (const binding of fix.bindings) {
            ...
            if (!node || !binding.field || !('setBoundVariable' in node))
                continue;
    ...
    for (const binding of fix.bindings) {
        if (binding.paintIndex === undefined)
            continue;
    ...
    for (const [nodeId, paintIndexes] of bindingsByNode) {
        const node = await getSceneNode(nodeId);
        const paints = fix.variableField === 'fill' ? ('fills' in (node ?? {}) ? node.fills : null) : ('strokes' in (node ?? {}) ? node.strokes : null);
        if (!node || !paints || !Array.isArray(paints))
            continue;
        let changedPaint = false;
        const nextPaints = paints.map((paint, index) => {
            if (!paintIndexes.includes(index) || paint.type !== 'SOLID')
                return paint;
            changedPaint = true;
        ...
        if (!changedPaint)
            continue;
        if (fix.variableField === 'fill' && 'setFillsAsync' in node)
            await node.setFillsAsync(nextPaints);
        if (fix.variableField === 'stroke' && 'setStrokesAsync' in node)
            await node.setStrokesAsync(nextPaints);
        changed += 1;
    }
    return changed;
}
